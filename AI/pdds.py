...
def perform_analysis(file):
    """Core analysis logic shared by HTML and JSON endpoints.

    Returns (result_dict, error_message_or_None).
    """
    if not file or file.filename == "":
        return None, "No image uploaded. Please select a file."

    if not allowed_file(file.filename):
        return None, "Invalid file type. Please upload a JPG, PNG, or WEBP image."

    img_bytes = file.read()

    if len(img_bytes) > MAX_FILE_SIZE_MB * 1024 * 1024:
        return None, f"Image too large. Max size is {MAX_FILE_SIZE_MB}MB."
    ...
    try:
        ...
        if pn_resp.status_code == 200:
            ...
            return None, "PlantNet rate limit reached. Try again tomorrow."
        else:
...
def analyze():
    ...
    result, error = perform_analysis(file)
    if error:
